...
@app.route('/admin_like/<int:video_id>', methods=['POST'])
def admin_like(video_id):
    user_email = session.get('email')
...
def view_saved_video_details():
    if 'email' in session:
        ...
        reg_id=session.get('reg_id')
        query='''
            SELECT register_user.email, videos.filename
            FROM register_user
            JOIN videos ON register_user.reg_id = videos.reg_id
            '''
        users = conn.execute(query).fetchall()
        return render_template('admin_head.html',content='view_saved_video_details.html', email=session['email'],users=users)
    return redirect(url_for('login'))
...
def admin_search():
    if 'email' in session:
        ...
        conn.close()
    # Render the template and pass the necessary data
        return render_template('admin_head.html', content='view_all_video.html',videos=videos,email=session['email'],rows=rows,images=images)
    return redirect(url_for('login'))
...
@app.route('/save_admin/<int:video_id>', methods=['GET', 'POST'])
def save_admin(video_id):
    if 'email' in session:
        conn = sqlite3.connect('user.db', timeout=50.0)
        cursor = conn.cursor()
        conn.row_factory = sqlite3.Row  
...
@app.route('/unsave/<int:video_id>', methods=['GET', 'POST'])
def unsave(video_id):
    if 'email' in session:
        conn = sqlite3.connect('user.db', timeout=50.0)
        cursor = conn.cursor()
        ...
        reg_id = session.get('reg_id')
        # Fixed SQL query for inserting a new comment
        cursor.execute('DELETE FROM save_video WHERE video_id=?',(video_id,))
        conn.commit()
